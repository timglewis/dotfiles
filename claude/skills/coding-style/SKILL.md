...
A bullet applies to any language unless it is marked with the language it belongs to.
...
- **C#**: prefer XML doc comments (`///`) over `//` line comments when documenting properties or fields, because they surface in IntelliSense. Reserve `//` for inline notes on non-obvious logic
...
- **C#**: prefer `catch (SpecificException e) when (<filter>)` over a broad catch, so unrelated failures are never intercepted
